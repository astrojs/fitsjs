...
describe "FITS Table", ->
  ...
  it 'can fix a problem with 555mos', ->
    ...
    runs ->
      ...
      expect(row[19]).toBeCloseTo(310.99219, 5)
      expect(row[20]).toBeCloseTo(311.0368, 4)
      expect(row[21]).toBeCloseTo(310.94769, 5)
      expect(row[22]).toBeCloseTo(-1.677314, 6)
      expect(row[23]).toBeCloseTo(3652.0701, 4)
      ...
      expect(row[36]).toBeCloseTo(5442.2261, 6)
      expect(row[37]).toBeCloseTo(522.32562, 6)
