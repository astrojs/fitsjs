...
describe "FITS Header", ->
  ...
  it 'can parse a FITS header describing an compressed image', ->
    ...
    expect(hdr1['EXTEND']).toBeTruthy()
    expect(hdr1['CHECKSUM']).toBe('3fB65ZA53fA53ZA5')
    ...
    expect(hdr2['PCOUNT']).toEqual(98583)
    expect(hdr2['GCOUNT']).toEqual(1)
    ...
    expect(hdr2['ZDITHER0']).toEqual(7362)
    expect(hdr2['CHECKSUM']).toBe('9RVcAOUa9OUaAOUa')
    expect(hdr2['DATASUM']).toBe('2081740934')
