...
(function() {
  ...
  Tabular = (function(_super) {
    ...
    Tabular.prototype.getColumn = function(name, callback, opts) {
      var cb, column, i, iterations, rowsPerIteration,
        _this = this;
      column = [];
      if (this.blob != null) {
        rowsPerIteration = ~~(4 * this.maxMemory / this.rowByteSize);
        rowsPerIteration = Math.min(rowsPerIteration, this.rows);
        iterations = ~~(this.rows / rowsPerIteration);
        i = 0;
        cb = function(rows, opts) {
          var c, startRow;
          console.log(iterations);
          c = rows.map(function(d) {
            return d[name];
          });
          ...
          if (iterations) {
            ...
            return _this.getRows(startRow, startRow + rowsPerIteration, cb, opts);
          } else {
            _this.invoke(callback, opts, column);
          }
        };
        return this.getRows(0, rowsPerIteration, cb, opts);
      } else {
        cb = function(rows, opts) {
