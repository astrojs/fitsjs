...
(function() {
  ...
  Base = (function() {
    ...
    Base.prototype.runCallback = function(callback, opts, data) {
      var context;
  ...
  Parser = (function(_super) {
    ...
    function Parser(arg, callback, opts) {
      ...
      if (typeof arg === 'string') {
        ...
        xhr.onload = function() {
          if (xhr.status !== 200) {
            _this.runCallback(_this.callback, _this.opts);
            return;
          }
      ...
      while (rows--) {
        ...
        if (arr[rowIndex] === 69 && arr[rowIndex + 1] === 78 && arr[rowIndex + 2] === 68 && arr[rowIndex + 3] === 32) {
          ...
          if (this.offset === this.length) {
            ...
            this.runCallback(this.callback, this.opts, this);
            return;
          }
          this.blockCount = 0;
  ...
  FITS = (function(_super) {
    ...
    function FITS(arg, callback, opts) {
      ...
      parser = new Parser(this.arg, function(fits) {
        ...
        return _this.runCallback(callback, opts, _this);
      });
    }
  ...
  Image = (function(_super) {
    ...
    Image.prototype.getFrames = function(frame, number, callback, opts) {
      ...
      cb = function(arr, opts) {
        ...
        frame += 1;
        if (number) {
          _this.getFrame(frame, cb, opts);
          return _this.runCallback(callback, opts, arr);
        }
      };
  ...
  Tabular = (function(_super) {
    ...
    Tabular.prototype.getColumn = function(name, row, number, callback, opts) {
      var accessor, byteOffset, column, columnIndex, descriptor, i, index, length, reader, rowByteSize, slice, _i,
        _this = this;
      columnIndex = this.columns.indexOf(name);
      rowByteSize = this.rowByteSize;
      descriptor = this.descriptors[columnIndex];
      length = this.offsets[columnIndex];
      byteOffset = rowByteSize * row;
      for (i = _i = 0; 0 <= columnIndex ? _i <= columnIndex : _i >= columnIndex; i = 0 <= columnIndex ? ++_i : --_i) {
        byteOffset += this.offsets[i];
      }
      byteOffset -= length;
      accessor = this.accessors[columnIndex];
      column = this.typedArray.hasOwnProperty(descriptor) ? new this.typedArray[descriptor](number) : [];
      if (this.blob != null) {
        reader = new FileReader();
        index = 0;
        reader.onloadend = function(e) {
          var offset, slice, value, view, _ref;
          view = new DataView(e.target.result);
          _ref = accessor(view, 0), value = _ref[0], offset = _ref[1];
          column[index] = value;
          if (index === number) {
            _this.runCallback(callback, opts, column);
            return column;
          }
          index += 1;
          byteOffset += rowByteSize;
          slice = _this.blob.slice(byteOffset, byteOffset + length);
          return reader.readAsArrayBuffer(slice);
        };
        slice = this.blob.slice(byteOffset, byteOffset + length);
        return reader.readAsArrayBuffer(slice);
      }
    };
  ...
  BinaryTable = (function(_super) {
    ...
    BinaryTable.prototype.setAccessors = function(header) {
      ...
      for (i = _i = 1, _ref = this.cols; 1 <= _ref ? _i <= _ref : _i >= _ref; i = 1 <= _ref ? ++_i : --_i) {
        ...
        _results.push((function(descriptor, count) {
          ...
          _this.offsets.push(_this.constructor.offsets[descriptor] * count);
          if (isArray) {
            switch (type) {
              case "COMPRESSED_DATA":
                accessor = function(view, offset) {
                  var arr, pixels, _ref1;
                  _ref1 = _this.getFromHeap(view, offset, descriptor), arr = _ref1[0], offset = _ref1[1];
    ...
    BinaryTable.prototype._getRows = function(buffer, nRows) {
      var accessor, index, offset, row, rows, value, view, _i, _len, _ref, _ref1;
  ...
  CompressedImage = (function(_super) {
    ...
    CompressedImage.prototype._getRows = function(buffer, nRows) {
      var accessor, arr, blank, data, i, index, nTile, offset, r, rIndex, row, scale, seed0, seed1, value, view, zero, _i, _j, _len, _len1, _ref, _ref1;
      view = new DataView(buffer);
      offset = 0;
